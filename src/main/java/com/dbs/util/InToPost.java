...
public class InToPost {
	...
	private Stack theStack;
	  ...
	  public String doTrans() {
	    for (int j = 0; j < input.length(); j++) {
	      ...
	      default: // must be an operand
	    	  output = Character.isLetter(ch) ? output + " " + ch : addSpace(input,j-1) ? output + " " + ch : output + ch ; // write it to output
	        break;
	      }
	    }
	  ...
	  public boolean addSpace(String input, int index) {
		  if(index == -1)
		      return false;
